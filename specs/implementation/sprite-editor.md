...
An example editor, for palettes, can be found in:

    /Users/jamesrandall/code/virtual-console/src/devkit/client/src/application/editors/palette/*

We want to create a sprite editor for our .gbin files. Create the sprite editor in the folder:

    /Users/jamesrandall/code/virtual-console/src/devkit/client/src/application/editors/spriteEditor

The sprite editor should have the following features:
...
* Allow a user to pick a palette file (.pbin) and palette from the file to use for the display of the sprite in the editor (this should be changeable while editing). Start with our default.pbin which is always present and with index 0.
...
* Support a tool palette which should be shown at the left of the editor:
    - Drawing tools:
        - Pixel pen (use the FontAwesome pen icon) 
        ...
        - Eraser (use the FontAwesome eraser icon) - this should work like then pixel pen but set the colour to index 0
    - Selection tools:
...
Its important to note that the sprites are stored row wise in the .gbin in 4bpp format - high nibble even pixels, low nibble odd pixels. The gbin format is an indexed colour format with the nibbles representing a palette index. 