; Static Starfield
; Draws 96 stars at fixed positions in three shades of gray
; Video Mode 0: 256x160 @ 4bpp
;
; Star distribution:
;   - 32 white stars (brightest)
;   - 32 medium gray stars
;   - 32 dark gray stars (dimmest)
...
main:
    ...
    ; Draw white stars (first 32)
    LD R0, #0          ; Start index
    LD R1, #32         ; Count
    LD R2, #COLOR_WHITE
    CALL draw_stars

    ; Draw medium gray stars (next 32)
    LD R0, #32         ; Start index
    LD R1, #32         ; Count
    LD R2, #COLOR_MED_GRAY
    CALL draw_stars

    ; Draw dark gray stars (final 32)
...
.loop:
    ...
    ; Get X coordinate from table
    LD R0, #(star_x_coords >> 8)
    LD R1, #(star_x_coords & $FF)
...
; Star position data - 96 X coordinates (pseudo-random distribution)
star_x_coords:
...
; Star position data - 96 Y coordinates (pseudo-random distribution)
star_y_coords:
